...
В рамках Программы и методики испытаний (ПМИ) мобильного приложения "Stets" версии 1.0 рассмотрены следующие основные функции:

1.  **Управление учетной записью пользователя:**
    *   **Регистрация:** Создание новой учетной записи пользователя (TR-1, TR-1.1, TR-1.2, TR-1.3).
    *   **Авторизация:** Вход в существующую учетную запись с использованием электронной почты и пароля (TR-2, TR-2.1, TR-2.2).
    *   **Восстановление пароля:** Процесс восстановления забытого пароля (TR-3, TR-3.1, TR-3.2).
    *   **Изменение электронной почты:** Изменение адреса электронной почты, связанного с учетной записью (TR-7.2).
    *   **Выход из учетной записи:** Завершение сеанса пользователя (TR-7.3).
2.  **Управление устройствами в доме:**
    *   **Добавление устройства:** Добавление нового устройства в систему (TR-4, TR-4.1, TR-4.2, TR-4.3, TR-4.4).
        *   Добавление с помощью QR-кода.
        *   Добавление вручную.
    *   **Удаление устройства:** Удаление существующего устройства из системы (TR-4.5).
3.  **Управление комнатами:**
    *   **Добавление комнаты:** Создание новой комнаты в доме (TR-5).
    *   **Изменение данных комнаты:** Редактирование информации о существующей комнате (TR-5.1).
    *   **Удаление комнаты:** Удаление существующей комнаты (TR-5.2).
    *   **Добавление устройства в комнату:** Привязка существующего устройства к определенной комнате (TR-5.3).
    *   **Удаление устройства из комнаты:** Отвязка устройства от комнаты (TR-5.4).
    *   **Управление всеми устройствами в комнате:** Включение/выключение всех устройств в комнате одновременно (TR-5.5).
    *   **Выбор энергосберегающего режима:** Настройка режима энергосбережения для устройств в комнате (TR-5.7).
4.  **Управление сценариями автоматизации:**
    *   **Создание сценария:** Создание нового сценария для автоматизации управления устройствами (TR-6, TR-6.1).
        *   Создание сценария с автоматическим запуском (по расписанию).
        *   Создание сценария с ручным запуском.
    *   **Удаление сценария:** Удаление существующего сценария (TR-6.2).
    *   **Запуск сценария:** Активация сценария (TR-6.3, TR-6.4).
        *   Принудительный запуск сценария с заданным расписанием.
        *   Запуск сценария с ручным управлением.
5. **Управление доступом к дому:**
    *   **Добавление пользователя для совместного управления домом:** Предоставление доступа другому пользователю к управлению домом (TR-7).
    *   **Принятие доступа:** Принятие приглашения для совместного управления домом (TR-7.1).
