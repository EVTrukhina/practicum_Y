...
1.  Необходимо доработать для Films Server API.
    1.  В ответ `{{FilmsServer}}/films/list` добавить новое поле `criticsRating`.
    2.  В ответ `{{FilmsServer}}/films` добавить новое поле `criticsRating`.

2.  Необходимо доработать для Series Server API.
    1.  В ответ `{{SeriesServer}}/series/list` добавить новое поле `criticsRating`.
    2.  В ответ `{{SeriesServer}}/series` добавить новое поле `criticsRating`.

3.  Необходимо доработать для Web Server API.
    1.  В запрос `{{WebServer}}/content/list` добавить два query-параметра: `sortBy` и `direction`.
    2.  В ответ `{{WebServer}}/content/list` добавить новое поле `criticsRating`.
    3.  В ответ `{{WebServer}}/content/details` добавить новое поле `criticsRating`.
